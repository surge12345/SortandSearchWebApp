...
def add_record():
    Student_ID = request.form['staff_id']
    ...
    cur.execute("""
        INSERT INTO staff (staff_id, name, surname, dateofbirth,address, phone, programme, gender , salary)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s , %s)
    """, (Student_ID, Name, Surname, Date_of_Birth,Address, Phone, Programme, Gender , Salary))
    conn.commit()
...
def get_field_index(field_name):
    field_names = ["staff_id", "name", "surname", "dateofbirth", "address", "phone", "programme", "gender" ,"salary"]
    try:
...
def sort_records():
    ...
    return render_template('records.html' ,records=sorted_records)  # Return sorted records template
...
def search_records():
    ...
    return render_template('records.html', records=search_results)
